---
title: Pager
description: Elements
layout: "guide"
---

<div id="pager">
	<div class="row">
		<div class="col-md-12">
			<h3>Default Pager</h3>

			<blockquote class="blockquote-sm blockquote-success">
				<p>Pager is a styled ul component to use with your pagination module.</p>
			</blockquote>

			<nav>
				<ul class="pager">
					<li><a href="#">Previous</a></li>
					<li><a href="#">Next</a></li>
				</ul>
			</nav>

			<nav>
				<ul class="pager">
					<li><span>Previous</span></li>
					<li><span>Next</span></li>
				</ul>
			</nav>

<div class="uxgl-toggle-code">
	<a class="btn btn-warning btn-xs" data-toggle="collapse" href="#codeCollapse0"><span class="icon-caret-down"></span></a>
</div>
<div class="collapse" id="codeCollapse0">
	<pre><code class="html">{literal}```<nav>
<ul class="pager">
    <li class="disabled previous"><a href="#">&larr; Older</a></li>
    <li class="active next"><a href="#">Newer &rarr;</a></li>
</ul>
</nav>```{/literal}</code></pre>
</div>

		</div>
	</div>

	<div class="row">
		<div class="col-md-12">
			<h3>Aligned Pager</h3>

			<blockquote class="blockquote-sm blockquote-success">
				<p>Align pager elements on the left and right side of its container with class <code>{literal}previous{/literal}</code> and <code>{literal}next{/literal}</code>.</p>
			</blockquote>

			<nav>
				<ul class="pager">
					<li class="previous"><a href="#">&larr; Older</a></li>
					<li class="next"><a href="#">Newer &rarr;</a></li>
				</ul>
			</nav>

<div class="uxgl-toggle-code">
	<a class="btn btn-warning btn-xs" data-toggle="collapse" href="#codeCollapse1"><span class="icon-caret-down"></span></a>
</div>
<div class="collapse" id="codeCollapse1">
	<pre><code class="html">{literal}```<nav>
    <ul class="pager">
        <li class="previous"><a href="#">&larr; Older</a></li>
        <li class="next"><a href="#">Newer &rarr;</a></li>
    </ul>
</nav>```{/literal}</code></pre>
</div>

		</div>
	</div>

	<div class="row">
		<div class="col-md-12">
			<h3>Disabled Pager</h3>

			<nav>
				<ul class="pager">
					<li class="disabled previous"><a href="#">&larr; Older</a></li>
					<li class="active next"><a href="#">Newer &rarr;</a></li>
				</ul>
			</nav>

<div class="uxgl-toggle-code">
	<a class="btn btn-warning btn-xs" data-toggle="collapse" href="#codeCollapse2"><span class="icon-caret-down"></span></a>
</div>
<div class="collapse" id="codeCollapse2">
	<pre><code class="html">{literal}```<nav>
    <ul class="pager">
        <li class="disabled previous"><a href="#">&larr; Older</a></li>
        <li class="active next"><a href="#">Newer &rarr;</a></li>
    </ul>
</nav>```{/literal}</code></pre>
</div>

		</div>
	</div>

	<div class="row">
		<div class="col-md-12">
			<h3>Pager Sizes</h3>

			<blockquote class="blockquote-success blockquote-sm">
				<p>Add <code>{literal}pager-sm{/literal}</code> or <code>{literal}pager-lg{/literal}</code> to any pager component to make it smaller or bigger.
			</blockquote>

			<p>Small</p>
			<nav>
				<ul class="pager pager-sm">
					<li class="previous"><a href="#">&larr; Older</a></li>
					<li class="active next"><a href="#">Newer &rarr;</a></li>
				</ul>
			</nav>

<div class="uxgl-toggle-code">
	<a class="btn btn-warning btn-xs" data-toggle="collapse" href="#codeCollapse3"><span class="icon-caret-down"></span></a>
</div>
<div class="collapse" id="codeCollapse3">
	<pre><code class="html">{literal}```<nav>
    <ul class="pager pager-sm">
        <li class="previous"><a href="#">&larr; Older</a></li>
        <li class="active next"><a href="#">Newer &rarr;</a></li>
    </ul>
</nav>```{/literal}</code></pre>
</div>

			<p>Default</p>
			<nav>
				<ul class="pager">
					<li class="previous"><a href="#">&larr; Older</a></li>
					<li class="active next"><a href="#">Newer &rarr;</a></li>
				</ul>
			</nav>

			<p>Large</p>
			<nav>
				<ul class="pager pager-lg">
					<li class="previous"><a href="#">&larr; Older</a></li>
					<li class="active next"><a href="#">Newer &rarr;</a></li>
				</ul>
			</nav>

<div class="uxgl-toggle-code">
	<a class="btn btn-warning btn-xs" data-toggle="collapse" href="#codeCollapse4"><span class="icon-caret-down"></span></a>
</div>
<div class="collapse" id="codeCollapse4">
	<pre><code class="html">{literal}```<nav>
    <ul class="pager pager-lg">
        <li class="previous"><a href="#">&larr; Older</a></li>
        <li class="active next"><a href="#">Newer &rarr;</a></li>
    </ul>
</nav>```{/literal}</code></pre>
</div>

		</div>
	</div>
</div>