...
<h3>SVG Icons as Background Images</h3>

<blockquote class="blockquote-sm blockquote-success">
	<p>We have created a Sass function to turn Lexicon SVG icon's into data uri schemes, which enables you to include them as a background-image on an HTML element. The Sass function <code>{literal}lx-icon(icon-name, color){/literal}</code> takes the name of a Lexicon SVG Icon and color takes a color. The Sass rule set <code>{literal}.site-add-cell { background-image: lx-icon(add-cell, #5AF); background-size: contain; display: inline-block; height: 20px; width: 20px; }{/literal}</code> will place the SVG icon add-cell as a background-image on any element with class site-add-cell.</p>
</blockquote>
...
<blockquote class="blockquote-sm blockquote-success">
	<p>You can turn your custom SVG into a data uri using the Sass function <code>{literal}svg-url($svg){/literal}</code> where <code>{literal}$svg{/literal}</code> is the code for your inline SVG. The function returns <code>{literal}url(your_svg_as_data_uri){/literal}</code>.</p>
</blockquote>

<div class="alert alert-info">
	When using <code>{literal}svg-url(){/literal}</code>, wrap your inline SVG code with single quotes if your SVG's attributes are delimited with double quotes.
</div>

<div class="row row-spacing">
	<div class="col-md-12">
		<span class="site-linux-tux site-svg-bg">Linux Tux</span>
	</div>
</div>


<h2>Why do we use SVG?</h2>
